...
REPLAY RULE — BINDING, AND IT APPLIES TO COMPREHENSION ONLY.
Needing a recording twice IS comprehension evidence, and the weight of that evidence
scales with how often it happened. One repeat on a genuinely hard clip is not the
same as needing half the recordings twice.

[EXAM_COMPLETE] carries COMPREHENSION_CAP, already computed from the total number of
replays across the exam. Treat it as a hard ceiling on COMPREHENSION:
  0-2 replays -> COMPREHENSION_CAP: 6   no cap, Level 6 reachable
  3-5 replays -> COMPREHENSION_CAP: 5
  6+  replays -> COMPREHENSION_CAP: 4
Do not recompute it and do not exceed it, however accurate the eventual answers were.
You may still award BELOW the cap if the answers warrant it — the cap is a ceiling,
not a score.

The other five descriptors are judged on the language the candidate produced, not on
how often they heard the audio. Whenever the cap binds, say so plainly in the
comprehension feedback and technical justification, naming which items needed a
second hearing.

YOU HAVE NO ACOUSTIC DATA ON THIS PATH.
This grades the text of the conversation only. There is no speech rate, no pause
length, no hesitation timing and no pronunciation confidence score. Do not invent
them and do not describe them as though you had measured them. Judge PRONUNCIATION
and FLUENCY only from what the text itself shows — false starts, transcribed
fillers, repetitions, broken syntax. Where the text cannot support a judgement, say
so in the justification rather than supplying one.
...
async function sessionValid(token) {
  ...
  if (!j) {
    console.warn('[auth] Apps Script gave no usable answer twice — refusing, role unknown.');
