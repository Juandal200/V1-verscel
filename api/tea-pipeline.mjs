...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Shared secret rather than a session: this is a server-to-server call, and
    // the candidate's session has no business authorising the filing of a result.
    // Apps Script accepts an unset secret so the two deploys can land in either
    // order; setting PIPELINE_SECRET in both places closes it.
    if (process.env.PIPELINE_SECRET) gasData.pipelineSecret = process.env.PIPELINE_SECRET;
    await saveToGAS({ action: 'apiSaveTEAResult', args: [gasData] });

    /* ── Return the student's view. Only the student's view. ──
     *
     * This used to send admin_view too — the annotated transcript and the
     * technical justification — to the candidate's browser. It was never needed
     * there: saveToGAS above has already filed the complete report, server to
     * server, before this line runs. The client was being handed a copy of
     * something it had no reason to hold and, until the guard added alongside
     * this change, drew a button to read it.
     *
