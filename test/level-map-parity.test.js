...
/* ── the map is real geodata, and the markers come from the projection ─────
 *
 * The hand-drawn silhouette is gone. What replaced it is generated by
 * tools/gen-map.mjs from world-atlas and committed as MapData.html — committed
 * rather than built, because Apps Script renders Index.html through HtmlService
 * and never runs build.js, so a build artefact would leave /exec with no map.
 *
 * Nothing here can look at the result. What it can check is that the markers are
 * placed THROUGH the projection rather than by hand, which is the failure that
 * made pins drift off their countries in the first place.
 */
console.log('--- the geodata is generated, committed, and included ---');
const MD = fs.readFileSync(__dirname + '/../MapData.html', 'utf8');
const IDX = fs.readFileSync(__dirname + '/../Index.html', 'utf8');
ok('MapData.html is generated, not hand-edited', /GENERATED by tools\/gen-map\.mjs/.test(MD));
ok('it carries the land path',    /land: 'M[\d.\-]/.test(MD));
ok('and the country borders',     /borders: 'M[\d.\-]/.test(MD));
ok('Index.html includes it',      /include\('MapData'\)/.test(IDX));
/* Order matters: Scripts reads window.LM_MAP at parse time. */
ok('before Scripts, which reads it',
   IDX.indexOf("include('MapData')") < IDX.indexOf("include('Scripts')"));

const MAP = new Function('window', MD.replace(/<\/?script>/g, '') + '\nreturn window.LM_MAP;')({});
ok('the stage is 1600 wide',      MAP.W === 1600);
/* H is deliberately taller than the land box — that gap is the open sea below
 * Australia, and collapsing the two would crop it. */
ok('and 885 tall, taller than the land',
   MAP.H === 885 && MAP.H > MAP.LAND_BOTTOM);
ok('the land path is substantial',  MAP.land.length > 20000, String(MAP.land.length));
ok('and so are the borders',        MAP.borders.length > 10000, String(MAP.borders.length));

console.log('--- every marker is placed through the projection ---');
const PROJECT = new Function('window', MD.replace(/<\/?script>/g, '') + '\nreturn window.LM_PROJECT;')(
  { LM_MAP: MAP });
const PLACE = new Function(grab('var _LM_PLACE = {') + '\nreturn _LM_PLACE;')();
const CP    = new Function(grab('var _LM_CP = {')    + '\nreturn _LM_CP;')();

/* Inside the box the land was fitted to. A marker outside it is in the margin or
 * in the open sea below Australia — which is what pixel offsets produced, and
 * what projecting the real coordinate is meant to stop. */
function onLand(pt) {
  return pt.x >= 20 && pt.x <= MAP.W - 20 && pt.y >= 24 && pt.y <= MAP.LAND_BOTTOM;
}
Object.keys(PLACE).forEach(function (code) {
  const p = PLACE[code];
  ok(code + ' projects onto the land area', onLand(PROJECT(p.lon, p.lat)));
});
Object.keys(CP).forEach(function (n) {
  const c = CP[n];
  ok('checkpoint ' + n + ' projects onto the land area', onLand(PROJECT(c.lon, c.lat)));
});
/* Sanity on the projection's orientation, because a sign error would still land
 * every point inside the box. */
ok('north is above south', PROJECT(0, 60).y < PROJECT(0, -30).y);
ok('east is right of west', PROJECT(120, 0).x > PROJECT(-120, 0).x);

/* And the RENDERER has to use it.
 *
 * Everything above proves the projection is right; none of it noticed when the
 * pin placement was swapped back for hand-rolled equirectangular arithmetic,
 * because the arithmetic still lands inside the box. This is the assertion that
 * catches that — and it is the whole point of the change, since offsets computed
 * beside the projection are exactly what drifted. */
const stageSrc = strip(grab('function _lmStageHtml(models, tiers, vrSlot, overlayHtml)'));
ok('pins are placed through the projection',        /_lmPt\(g\.place\.lon, g\.place\.lat\)/.test(stageSrc));
ok('checkpoints too',                               /_lmPt\(at\.lon, at\.lat\)/.test(stageSrc));
ok('and nothing computes a position by hand',       !/\/\s*360\s*\*|_lmX\(|_lmY\(/.test(stageSrc));
ok('the old equirectangular helpers are gone',      !/function _lmX|function _lmY|_LM_LAT_T/.test(S));

console.log('--- and nothing on this screen is dotted ---');
const sheet = fs.readFileSync(__dirname + '/../Styles.html', 'utf8')
  .replace(/\/\*[\s\S]*?\*\//g, '');
ok('no stroke-dasharray survives', !/stroke-dasharray/.test(sheet));
ok('land is filled, so the outline can be a hairline',
   /\.lm-land\s*\{[^}]*fill:\s*#171d21/.test(sheet));
ok('borders are not filled',   /\.lm-borders\s*\{[^}]*fill:\s*none/.test(sheet));
/* .lm-stage is scaled by transform to fit the window; without this a 0.8px line
 * thickens and thins with the viewport. */
ok('both strokes are non-scaling',
   (sheet.match(/vector-effect:\s*non-scaling-stroke/g) || []).length >= 2);
